...
//		System.out.println("[BOT-2]: " + getBottom2ClassName() + ", " + getBottom2MethodName() + ", " + getBottom2MethodLine());
	...
	public double[] showSTFeatures(){
		...
		int exceptName = getExceptionType(getExpName());
		...
		int methodNum = getMethodNum();
		/**ST05: Whether an overloaded method exists in the stack trace*/
		int isOverLoaded = getisOverLoaded()?0:1;
		/**ST06: Length of the name in the top class*/
		int lenTopClassName = RepsUtilier.getSimpleClassName(getTopClassName()).length();
		/**ST07: Length of the name in the top function*/
		int lenTopMethodName = getTopMethodName().length();
		/**ST08: Length of the name in the bottom class*/
		int lenBottomClassName = RepsUtilier.getSimpleClassName(getBottomClassName()).length();
		/**ST09: Length of the name in the bottom function*/
		int lenBottomMethodName = getBottomMethodName().length();
					
		double[] features = {exceptName,loc,classNum,methodNum,isOverLoaded,lenTopClassName,lenTopMethodName,lenBottomClassName,lenBottomMethodName};
